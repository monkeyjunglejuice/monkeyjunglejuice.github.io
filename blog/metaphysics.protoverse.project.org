...
Of course that's not the whole story: Certainly, humans are developing tools and methods like mathematics and technology to overcome their limitations. But a closer look reveals that most methods are built on the the same substance-materialist preconceptions, serving substance-materialist purposes.
...
We'll need to consider a paradigm shift: a worldview constituted from /process, relation and transformation/, to finally get out of the stuffy theater where particles dance to the beat of time on a stage of space.
...
Observers arise on a spectrum, and they are pervasive. At this point we can generalize 'observers' into /Processes/, which we will use as the prime abstraction within the Protoverse.
...
Either way, I envision (primitive) Process as a meta-framework that describes Processes as the potential interactions between Processes. Yeah, it's self-referential/recursive -- to understand recursion, you'll first need to understand recursion.

** Necessary Process Characteristics

#+begin_index
- There's a problem with "fundamental" and "first principles": It would be quite unwieldy trying to figure out initial rules that produce a viable universe. The way out: there must be intrinsic adaption. Hence I lean towards the principle of Universal Darwinism (more about that later).
...
- Absolute compatibility in terms of interaction between higher-order Processes is (only) guaranteed at the Scale of their constituent primitive Processes. Higher-order Processes may develop various degrees of interaction -- e.g. more effective, efficient or richer "implementations", which may deviate significantly from the interactions of primitive Processes; e.g. they might interact via evolved, commonly shared protocols or languages. This leads to differentation in interaction capabilities and encapsulation; meaning not everything interacts with everything else to the same extend. Example: a hydrogen atom can bond with a carbon atom, but you can't procreate with a piece of wood, even though both of you are made from the same basic stuff.
#+end_index

** Low-level Process Interaction

The precursor for interaction between processes is the /Experience/. It seems like a suitable term, stolen from Whitehead's Process Philosophy. What does a Process experience? Patterns produced by other Processes, because there isn't anything other than Processes (no backdrop or stage-like "environment" -- Processes continuously /become/ (co-evolve) their environment).
...
Experience may be tied to something changing (or can there be anything experienced if nothing changes at all?), and therefore registration of change. So what is even 'change' itself? Surely, the term implies the notion of temporality -- before/after. It seems that 'change' means experiencing 'difference'.
...
The key question would be: Do the relationships between Processes last forever, once established? Which would lead to everlasting histories. Or can they be dissolved or transformed, e.g. =(A -> B -> C)= reduced to =(A -> C)=? Or shadowed? Under what /neccessary and organically arising/ circumstances?
  ...
  That's not the point here ... focus, please. It seems obvious though, that higher-order Processes (Processes consisting of Processes) can perish -- by what means, exactly? Does that happen by /transformations/ or /deletion -- complete loss/? Some Processes dissolve, others persist, and new ones establish at that moment. What will be left by this impact? How far through the scales of Processes reaches the impact? What about the primitive Processes?
...
What are the necessary principles of Darwinian Evolution? I'm using generic terms to minimize substrate-specific connotations (e.g., replacing "inheritance" with "Retention", because "inheritrance" describes just a mere implementation detail rather than the general concept):
