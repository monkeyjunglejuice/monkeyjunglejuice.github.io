...
Well first, the Protoverse isn't supposed to be a "simulation" to begin with, but the instatiation of an abstract model. It's not going to simulate /something else/ -- but will be "the real thing" itself.
...
Global time doesn't exist in reality, violates the process-relational Protoverse metaphysics (no background time, no background space -- these are emergent phenomena), and regarding efficient computation, synchronous progression is probably the worst possible decision, as it kills parallelization and scaling. Maybe the universe is so efficient because it's inherently asynchronous.
...
- Need to "synchronize" only what actually interacts
