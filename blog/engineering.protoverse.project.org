...
Before we come to concurrency and parallelism: the concepts are often mixed up, and understandably so -- [[https://wiki.haskell.org/index.php?title=Parallelism_vs._Concurrency][here's an explanation]] and [[https://www.classes.cs.uchicago.edu/archive/2017/spring/22300-1/lectures/ConcurrencyParallelism/ConcurrencyParallelism.pdf][here]]. Parallelism is the thing that we really want.
...
b) Adher to portable code, that means following a standard closely (e.g. R6RS for the Scheme language).
...
- Parallelism: [[https://docs.racket-lang.org/guide/parallelism.html][Places]] looks quite awkward, bolted-on and fully manual;
...
/Sentiment: Gerbil Scheme is a promising project but in a very early stage (v18.0), and doesn't support multiple CPU cores. The interactivity and Emacs integration isn't comparable to Guile's, e.g. documentation or code autocompletion./
...
*** Lua
- Simple, versatile and multi-paradigm, super expressive
...
- There are workarounds, e.g. Fennel as a lispy language on top of Lua;
- Has tail call optimization, which is nice;
- Single-core concurrency only;
- I couldn't figure out how to parallelize Lua programs in reasonable time
- There are some attempts to make it "multicore", but not maintained;
...
- I ran into various errors when I did trivial benchmarks involving 100k concurrent computations via core.async "go blocks"; depending on which concurrency facility used, I ran constantly into some limitations when it came to large numbers of concurrent operations;
...
- No conurrent garbage collector but stop-the-world;
...
- Aims to provide fully transparent "automatic" parallelism, no manual managment of parallelization (no thread creation, locks, mutexes, etc.);
...
I'm not expecting to plug libraries together (how most "software engineering" is done today), but to go through a journey of discovery and exploration, and to think through the whole process "from scratch", and the language should support that.
...
Most "scientific" projects are currently done in Python, in combination with certain libraries that are written in C and C++ responsible for the heavy lifting; and there's a huge amount done in Java, too (and Scala to some degree) -- but I don't really want to work with these languages, as these are all quite complicated, verbose and ugly); even Rust doesn't seem to be a great option either (verbose syntax, low-level, manual). So?

*** The Finalists

I'd like to consider two languages/platforms for the role of the primary language/runtime.

What they have in common:

- Deep recursion is idiomatic (no stack overflows);
...
The actor model - available at the language level - makes it convenient to write concurrent programs, and the BEAM parallelizes automatically where it sees fit. Each Erlang process has it's own stack, heap and garbage collector - and you can have a million or more of these processes in a single node. Overall, the GC story is pretty compelling.

The BEAM is not very performant for CPU-bound-, but for IO-bound computing. E.g. Guile turns out twice as fast as Elixir (benchmark: sequential, non-tail-recursive algorithms on binary trees with ~67 million nodes). However, CPU-intensive parts could be ported to high-performance implementations where necessary.

The BEAM is rather "infrastructure" and "industry" centred, and its languages require quite a bit of ceremony and overall, the framework character (OTP) shines through - it's rather "translate your problem to OTP" rather than "mold your language to fit the problem". That feels a bit clunky for ad-hoc experimentation -- on the other hand, people are capable to write quite sophisticated things in something like C++. Probably I should take a second look at LFE though.
...
It may be surprising, but Guile Scheme looks quite feasible as a candidate for the primary language, because it has become much more than just an "extension language" for "scripting applications written in C". For instance, it seems to handle large data structures and hefty allocation quite well -- out-of-the-box, in the REPL.

The GC (as of version 3.0.10) seems to need less attention compared to SBCL's ccurrent GC (version 2.5.10). When I did some benchmarks with deep non-tail-recursion on binary trees with ~67 million nodes (sequential), Guile turned out 1.5 times faster than SBCL, which spent 3/4 of the time with GC. Overall, Guile feels well balanced and therefore quite fast, suited for writing code ad-hoc without ceremony.

Guile's focus is less on infrastructure (like the BEAM languages), and more on expressivity (and exploration), which might be beneficial in the early stages. Nevertheless, there's also stuff going on with distributed computing (e.g. Spritely Goblins).
