...
* Paradigms, Structures and Models of Computation
...
Aside from that, I'm quite fond of continuous structures at the lowest level, because necessary successive discretization could elegantly introduce notions of approximation and uncertainty, [[file:metaphysics.protoverse.project.org][as mentioned in the metaphysics]]. I'm thinking of a "level of detail" (LoD) approach, where the discretization happens according to a Process' perspective or epistemological constraints; the discretization would be defined by relationships between Processes rather than by fixed spatial or temporal grids. Let's keep that aside right now, and look at some paradigms, models of computation and associated structures.

** Cellular Automata

Cellular automata allow for efficient parallel computation in theory, because each cell's state depends only on its local neighboorhood, and their states are updated synchronously -- well, usually. Basically, CAs can also have asynchronous time progression, have irregular grids, multiple dimensions and arbitrary many states per cell. But if we venture that far, we'll get into territories where other structures allow more flexible ways to encode data and dynamics.

** Graphs / Hypergraphs

Graphs offer more flexibility to encode also non-local relations and interactions. Graphs introduce a fundamental duality: nodes and edges. Graphs emphasize relations, networks, recursive and fractal-like structures, which corresponds well to modeling Nested Scales of Complexity. And since the Protoverse is supposed to be process-relational and encode various types of relationships across various scales, hypergraphs seem like a perfect match because of their capability to encode multi-way relationships.

*** Interaction Nets (Interaction Combinators)

Stumbled upon [[https://en.wikipedia.org/wiki/Interaction_nets][Interaction Nets]], a model of parallel graph rewriting with strong confluence and locality, which has been implemented as the [[https://github.com/HigherOrderCO/HVM][HVM]] and others. On a first glance, the idea of Interaction Nets seems like an interesting runtime for the Protoverse. It's low-level, so not modeling-friedly, but it its operational semantics correspond well, as computation (Protoverse: evaluation) happens through interaction (Protoverse: Observation or Experience).

** Functions

The idea here is to shift the focus from a pre-defined underlying structural substrate like cellular automata and graphs to just functions for dynamically evolving structures. Eventually, we may well end up with some kind of implicit graph anyway (or an unintelligible mess), because we're just abstracting the notion of structure and algorithms working along these structures into the behavior of functions themselves.

** Language Oriented

The idea here is to have a language representation that is subject to be utilized and evolved directly by the Protoverse Processes ("data is code, code is data"). Everything then would happen at the language level, abstracting from stuff like functions, data structures, graphs, etc. That would certainly add semantic denseness.

** Process Calculi

As per my current understanding, designing the Process dynamics would result in something like a [[https://en.wikipedia.org/wiki/Process_calculus][process calculus]] anyway.

Process calculi are considered models of computation; they are formal systems to describe, analyze and express process dynamics, e.g. process calculi are used to model dynamics in biology.
...
What type of application will the Protoverse become? There are basically two approaches, and each one calls for a different development style, choice of language and platform, different analysis methods, maintenance- and scaling strategies, and a plethora of other requirements.
...
Pivoting quickly into different directions must be feasible, so we also want solid refactoring capabilities.
...
A lot of stuff is going to happen. Eventually I'm expecting many billions of smallish, interacting computations. In the early stages, where the focus is on modelling the Process abstraction and dynamics, the capacity to handle a couple of millions might be enough, which should be achievable in the cloud, on a small Rasperry Pi cluster or even a bunch of laptops.
...
There are basically two facilities in common use that fit massive concurrency: Either [[https://en.wikipedia.org/wiki/Actor_model][actor system]] implementations (the Erlang model) or CSP-style channels (the Concurrent ML or Golang model) -- [[https://wingolog.org/archives/2016/10/12/an-incomplete-history-of-language-facilities-for-concurrency][breakdown here]].

My first naive idea was to map primitive Protoverse Processes and runtime threads one-to-one, where the runtime threads are scheduled by a runtime and multiplexed to kernel threads. Tying high-level dynamics 1:1 to the confinements of an underlying implementation is probably not the best idea.

Here is to note that Protoverse Processes compose into higher-order Processes, which means that way higher-order Protoverse Processes would get automatically more "computational power", when the lowest-level Processes are tied to runtime processes/threads or whatever (e.g. Erlang/BEAM processes).

However, to model Protoverse Processes (the "Entities" so to speak) directly via runtime threads/processes may lead to scaling issues pretty quickly, so it would rather make more sense to decouple both and model Protoverse Processes as /data/ instead, and use (ephemeral) runtime processes as an abstracted-away layer that merely "carries" or "supports" their dynamics.
...
- [[https://github.com/opencog/atomspace][OpenCog Atomspace]]: possible backend for the Protoverse with Guile interface; it's a in-RAM generalized hypergraph store and rewriting system;
...
Therefore it seems quite likely that I may not end up modeling Protoverse dynamics directly in the Actor Model, but rather a "Protoverse Calculus" that would have to be ported to run on the BEAM or others.
