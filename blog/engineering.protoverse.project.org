...
- Ther are [[https://github.com/marcoheisig/Petalisp][Petalisp]] (data parallelism), Lparallel and [[https://github.com/marcoheisig/cl-mpi/][OpenMPI bindings]] (likely an overkill, see [[https://www.open-mpi.org][OpenMPI]]);
...
- Aims to provide fully transparent (implicit?) parallelism, no manual managment of parallelization (no thread creation, locks, mutexes, etc.);
...
- Theoretically, it also could support Guile with [[https://codeberg.org/spritely/hoot][Hoot]]; but that's a lot of work;
- Still experimental, and seems abandoned too;

** Conclusion

The Protoverse project ventures into uncharted territory, and my gut feeling says that I should counterbalance that notion with adhering to mature, "industrial-grade" technology where possible.
...
Most "scientific" projects are currently done in Python, in combination with certain libraries that are written in C and C++ who are doing the heavy lifting; and there's a huge amount done in Java, too (and Scala to some degree) -- but I don't really want to work with these languages (they are all quite complicated, verbose and ugly); even Rust doesn't seem to be a great option either (verbose, low-level, manual). So?
...
The Erlang platform, using either Elixir and/or Erlang, follows a connectivity-based approach (message passing): The BEAM is not very performant for CPU-bound-, but for IO-bound computing. However, CPU-intensive parts could be later ported to high-performance implementations if that's necessary.
