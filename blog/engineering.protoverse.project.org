...
The Protoverse project ventures into uncharted territories, and my gut feeling says that I should counterbalance that notion with adhering to mature, "industrial-grade" technology.

I'm not expecting to plug libraries together (how most "software  engineering" is done today), but to go through a journey of discovery and exploration, and think through the whole process "from scratch" by myself. I am aware that I'll be "re-inventing the wheel" several times, which certainly will lead to inefficient, naive implementations (evolutionary algorithms are such a candidate, for instance).

In such cases it will be desirable to draw from established algorithms/libraries, after I have developed an understanding beforehand.

*The finalists:*

1. The Erlang platform -- using the Elixir and/or the Erlang language -- might be not as performant for "raw computation", but supports the actor model at the language level and therefore makes it really convenient to write highly concurrent programs, which could be later ported to high-performance implementations if necessary;

2. Julia, even though being an imperative language, offers much convenience and is backed by a substantial ecosystem focused on scientific computing;
