...
The problem with current architectures is, that parallelism is tied to the number of /physical/ cores available. The ideal thing to have would be something like "soft-scalable" parallel concurrency, but in practical terms with current architecture we need both, concurrency and parallelism.
