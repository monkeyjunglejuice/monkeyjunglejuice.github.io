...
In any case, it makes sense to have straightforward GPU computing readily available (OpenCL or CUDA) if needed.
...
Sentiment: It's my favorite language actually, but the tooling (Opam, Dune build system, Ocaml REPL and Utop REPL) is such a horrible mess. Well and there's no real interactivity anyway.
...
** Elixir vs. Guile Scheme:
Both embrace recursion. Guile Scheme would be interesting if it turns out performant enough in a highly concurrent/parallel setting.

** Julia vs. Common Lisp
Both are actually pretty similar in many regards; the bigger ecosystem gives Julia an edge, while Common Lisp supports conversational programming at the running program.
