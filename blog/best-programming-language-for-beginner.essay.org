#+TITLE: Best Programming Language for absolute Beginners? (2025 Update)
#+DATE: <2020-06-25 Thu>
...
But usually you want it on your laptop. There are plugins for [[https://atom.io/packages/slima][Atom]], [[https://marketplace.visualstudio.com/items?itemName=ailisp.commonlisp-vscode][VSCode]] and [[https://github.com/vlime/vlime][Vim]], although I recommend Emacs, which is straightforward to install [[https://github.com/doomemacs/doomemacs][via Doom Emacs]].
...
Elixir is new language that runs on the Erlang platform. Elixir is mostly used for writing server-backends for web apps, but it's also pretty neat for writing little scripts and automating everyday's tasks.
...
What I dislike about Elixir is that many constructs have 3-4 "shorthands" or special cases to write the same thing, a.k.a. "syntactic sugar". It looks a bit like the language designers couldn't decide what ideas to keep and what to throw out -- so it is often less clear what you are looking at.
...
JavaScript is an imperative language at the very heart, but you can write object-oriented and functional style too. JavaScript assimilates new features with each version and therefore it's getting bigger and bigger. Resistance is futile. Or is it?
...
OCaml is in the same family as Elm and Haskell, but has less ideological ballast. OCaml shines where speed and correct code are are important; like financial technology, blockchain or compilers. [[https://ocaml.org/learn/companies.html][Some bigger companies]] are heavily invested in OCaml. Some universities use OCaml to teach functional programming, so I found amazing learning material for absolute beginners.

OCaml syntax is quite similar to Haskell's syntax, but leaves more room to breathe. It looks well structured, what makes it easy to recognize patterns in the code to know what's going on. I'd say consistency and elegance are it's most remarkable attributes. So far, I've written a couple trivial programs in it, and still had no WTF moments yet (and I'm really sensitive to that regard).

While OCaml the language itself is a beauty, the tooling is not as nice. Getting Opam (the package manager), Dune (the build system) and one of the REPLs (Ocaml or Utop) working together is poorly documented, spread over several websites and involves a lot of ceremony -- and there's always something not working in surprising ways.

Nevertheless, I'd consider it very suitable as the first programming language, because it's unlikely that your learning projects spread out over multiple files and use several libraries.
...
If you want a language which is easier to learn, compact and highly performant, you may have fun with [[*Julia as Your First Programming Language][Julia]]. And if popularity is still such a thing for you after [[*Popularity and Commmunity][reading this]], then learn Python of course, but better as your /second/ language.
...
Julia is being developed as a pragmatic high performance vehicle for technical and scientific computing, as an alternative to Python, Fortran, R and other languages used in the fields. I would say its a language to build neither ready-to-ship software, but more like a system to run your computing tasks in. Julia strives to present a polished experience.

It's still a young language, but eagerly promoted by a couple of organizations. So it looks like there's more of an ecosystem than you would expect for a language in that early state.

Julia's REPL is quite powerful (maybe second to Common Lisp's REPL) and allows a high level of interactivity. You basically don't have to leave the REPL to do anything, which is awesome.

The language looks friendly and easy to start with, and you should definitely check it out if you're a student and work in machine learning, data science, statistics, numerical computing, mathematics or just want to plot some stuff. You might have heard about Jupyter notebooks -- that's where the "Ju" comes from.

The learning material is targeted for researchers who stand on the brink to make the switch. So for now, don't expect "learn programming" books for total beginners, but there are several online courses, if that's your thing.
...
But Common Lisp is no functional language, and I wanted to expose myself primarily to that way of programming. In Common Lisp, You'll encounter mostly projects written in object-oriented style. But since I'm inclined towards functional programming (it's way simpler!), I jumped ship to [[*OCaml as Your First Programming Language ][*OCaml*]] as my first programming language.
